...
dfLB = pd.read_excel("data_ETHBUSD_5m.xlsx")
if len(ohlcvLB):
...
for i in range(zigzag_len-1, int(np.size(data) / 4)):
    ...
    print(data[i-zigzag_len+1:i+1, 1])
    print(data[i-zigzag_len+1:i+1, 2])
    
    print(open, high, low, close,highest, to_up, lowest, to_down)
    exit()
