...
class JawWorm(Monster):
    ...
    def action(self):
        ...
        self.block += math.ceil(self.damage_taken/2)
        self.damage_amount = self.damage_taken//2
        return {
            'damage': self.damage_amount}

class Encounter:
    def __init__(self, player:Player, monster:list):
      self.player = player
    ...
    def enemy_turn(self) -> None:
        for monsta in self.monster:
            if 'weak' in monsta.action().keys():
                self.player.add_weak(monsta.action()['weak'])
            if 'vulnerable' in monsta.action().keys():
                self.player.add_vulnerable(monsta.action()['vulnerable'])
            if 'strength' in monsta.action().keys():
                monsta.add_strength(monsta.action()['strength'])
        
            totalDamage = monsta.action()['damage'] + monsta.get_strength()
...
def main():
    ...
    for encounter in encounterAct:
        ...
        for monster in encounter:
            ...
            elif monster[0] == "Louse":
                listMonster.append(JawWorm(monster[1]))
        
        gameEcounter = Encounter(player,listMonster)
