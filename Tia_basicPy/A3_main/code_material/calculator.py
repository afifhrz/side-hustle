...
class CalcApp():
    def __init__(self, master: tk.TK) -> None:
        ...
        def handler():
            try:
                self._expression = str(eval(self._expression))
                self._refresh()
                print(f"pressed =.  Expresssion {self._expression}")
            except:
                self._expression = "ERROR.  PRESS C TO RESET"
                self._refresh()
    ...
    def _make_button(self, root: tk.Tk, cs: str) -> tk.Button:
        def handler():
            self._expression += cs
            self._refresh()
            print(f"pressed {cs}.  Expresssion {self._expression}")
